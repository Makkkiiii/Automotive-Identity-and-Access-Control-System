...
Phase 10.1.9 fixes Provisioning page cloud hydration for existing completed records. Selecting an existing cloud-backed customer, vehicle, and key fob now refreshes safe vehicle/key-fob status metadata, loads issued certificate metadata, applies the latest finalized provisioning session, and updates the visible workflow/status cards without reusing stale demo certificate, proof, or session state. New selections still clear incompatible runtime artifacts, and only safe IDs, statuses, fingerprints, and redacted metadata are displayed.
